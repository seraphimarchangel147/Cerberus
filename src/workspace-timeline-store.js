...
function safeLstat(file) {
  try {
    ...
    // ENOENT: the path is absent. ENOTDIR: an ancestor is currently a FILE, so
    // this path cannot exist either — that happens legitimately during a
    // file<->directory type transition (e.g. travelling back to a snapshot where
    // "shape" was a file while the workspace has "shape/child.txt"). Both mean
    // "nothing here to preflight"; only genuine errors should propagate.
