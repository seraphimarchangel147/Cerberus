...
test("project tests use Node permissions and cannot read sibling workspaces", async (t) => {
  ...
  // The security guarantees above (run failed, no escape write, no secret in
  // output) are the actual contract. The wording below is Node-version
  // dependent: since Node 25 the `--test` runner cannot enumerate its target
  // under `--permission` and aborts with "Could not find <file>" before the
  // access-denied path is reached. Verified separately that a direct
  // `readFileSync` of a sibling project under the same flags still raises
  // ERR_ACCESS_DENIED, so isolation itself is intact on this Node.
  assert.match(
    result.tail,
    /restricted|access denied|ERR_ACCESS_DENIED|Could not find/i
  );
});
